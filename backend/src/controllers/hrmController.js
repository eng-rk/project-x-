...
exports.createLeaveRequest = async (req, res) => {
  try {
    ...
    if (leaveType === 'Unpaid') {
      ...
      paidDays   = Math.min(daysCount, remaining);
...
exports.updateCandidateStatus = async (req, res) => {
  try {
    ...
    // Onboarding hook: create User and draft Contract when status transitions to 'Hired'
    if (status === 'Hired') {
      ...
      let contract = await Contract.findOne({ employeeId: user._id });
      if (!contract) {
        contract = await Contract.create({
          employeeId: user._id,
          baseSalary: 10000,
          ...
          salaryHistory: [{
            amount: 10000,
            changedBy: req.user._id,
            reason: 'Draft initialized from hiring candidate'
          }]
        });
      }
    }
