...
exports.generatePayrollRun = async (req, res) => {
  try {
    ...
    for (const c of contracts) {
      ...
      // --- Custom Contract Salary Components & Fallback Allowance Tiers ---
      ...
      let totalAllowancesEmp = 0;
      let totalDeductionsEmp = 0;

      if (c.salaryComponents && c.salaryComponents.length > 0) {
        for (const comp of c.salaryComponents) {
          const val = comp.valueType === 'Percentage' ? (comp.value / 100) * baseSalary : comp.value;
          if (comp.type === 'Earning') {
            totalAllowancesEmp += val;
            const label = comp.label.toLowerCase();
            if (label.includes('transport')) transportAllowance += val;
            else if (label.includes('housing')) housingAllowance += val;
            else if (label.includes('meal')) mealAllowance += val;
            else if (label.includes('mobile')) mobileAllowance += val;
            else if (label.includes('fuel')) fuelAllowance += val;
          } else if (comp.type === 'Deduction') {
            totalDeductionsEmp += val;
            const label = comp.label.toLowerCase();
            if (label.includes('pension')) pension += val;
            else if (label.includes('advance')) advanceDeduction += val;
            else otherDeductions += val;
          }
        }
      } else {
        // Fallback to hardcoded tiers if contract has no components defined
        transportAllowance = baseSalary >= 10000 ? 500  : 300;
        mealAllowance      = baseSalary >= 10000 ? 300  : 150;
        mobileAllowance    = 200;
        housingAllowance   = baseSalary >= 20000 ? 2000 : 0;
        fuelAllowance      = emp.role?.includes('Manager') || emp.role?.includes('Director') ? 1000 : 0;
        totalAllowancesEmp = transportAllowance + mealAllowance + mobileAllowance + housingAllowance + fuelAllowance;
      }

      // --- Leave without pay ---
      // Only count leave requests that are approved and of type 'Unpaid'
      const empLeaves = leaveRecords.filter(l => l.employeeId?.toString() === emp._id?.toString() && l.leaveType === 'Unpaid');
      let lwpDays = 0;
      ...
      const loanDeduction = empLoan ? Math.min(empLoan.monthlyInstallment, empLoan.remainingBalance) : 0;
      ...
      const totalDeds     = incomeTax + socialIns + loanDeduction + lwpDeduction + totalDeductionsEmp;
...
exports.releasePayrollRun = async (req, res) => {
  try {
    ...
    const entries = await PayrollEntry.find({ runId: run._id }).populate('employeeId', 'firstName lastName email');
    const now = new Date();
    let paidCount = 0;
    let failedCount = 0;
    ...
    for (const entry of entries) {
      if (!entry.employeeId) {
        entry.status = 'Failed';
        entry.failureReason = 'Employee record missing or deleted';
        await entry.save();
        failedCount++;
        continue;
      }
      ...
      try {
        ...
        try {
          if (entry.employeeId.email) {
            sendEmail(
              entry.employeeId.email,
              `Payroll Disbursed: ${run.period}`,
              `Hello ${employeeName},\n\nYour salary for the period ${run.period} has been successfully disbursed.\nNet Pay: EGP ${entry.netSalary.toLocaleString()}\n\nBest regards,\nHR Team`
