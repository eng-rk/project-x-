...
exports.getSystemAnalytics = async (req, res) => {
  try {
    ...
    const convertedLeads = await Lead.countDocuments({ status: { $in: ['Confirmed', 'Won', 'Converted'] } });
    ...
    const prevConvertedLeads = await Lead.countDocuments({ status: { $in: ['Confirmed', 'Won', 'Converted'] }, createdAt: { $lt: oneMonthAgo } });
      ...
      { $unwind: '$campaignInfo' },
      {
        $group: {
          _id: {
            ...
            status: {
              $cond: {
                if: { $in: ['$status', ['Confirmed', 'Won', 'Converted']] },
                then: 'Converted',
                else: '$status'
              }
            }
    ...
    const teamPerformance = await Promise.all(
      teamMembers.map(async (member) => {
        ...
        const leadsConverted = await Lead.countDocuments({ assignedTo: member._id, status: { $in: ['Confirmed', 'Won', 'Converted'] } });
...
exports.getMarketingPerformance = async (req, res) => {
  try {
    ...
    if (performanceData[0].conversionRate > performanceData[1].conversionRate) {
      ...
      // Tie breaker based on total leads volume
